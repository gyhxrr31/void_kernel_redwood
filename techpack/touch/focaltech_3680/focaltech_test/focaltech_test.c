...
ssize_t tp_selftest_write(struct file *file, const char __user *buf,
			  size_t count, loff_t *pos)
{
	char tmp[6];
	int ret;
...
static int32_t datadump_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; (i < tdata->node.tx_num) && (i < TX_NUM_MAX); i++) {
		for (j = 0; (j < tdata->node.rx_num) && (j < RX_NUM_MAX); j++) {
			seq_printf(m, "%6d",
				   differ_data[tdata->node.rx_num * i + j]);
			if (j == (tdata->node.rx_num - 1))
				seq_printf(m, "\n");
		}
	}
...
int fts_test_init(struct fts_ts_data *ts_data)
{
	...
	proc->tp_selftest_proc =
		proc_create("tp_selftest", 0644, NULL, &tp_selftest_fops);
	if (proc->tp_selftest_proc == NULL)
		FTS_TEST_ERROR("tp_selftest proc create failed.");

	proc->tp_data_dump_proc =
		proc_create("tp_data_dump", 0444, NULL, &tp_datadump_fops);
